...
public class EhCacheManager implements CacheManager, Initializable, Destroyable {
	...
	/**
	 * Indicates if the CacheManager instance was implicitly/automatically created
	 * by this instance, indicating that it should be automatically cleaned up as
	 * well on shutdown.
	 */
	...
	/**
	 * Returns the wrapped Ehcache {@link net.sf.ehcache.CacheManager CacheManager}
	 * instance.
	 *
	 * @return the wrapped Ehcache {@link net.sf.ehcache.CacheManager CacheManager}
	 *         instance.
	 */
	 ...
	 * EhCache CacheManager instance. The string can be any resource path supported
	 * by the
	 * {@link org.apache.shiro.io.ResourceUtils#getInputStreamForPath(String)} call.
	 * <p/>
	 * This property is ignored if the CacheManager instance is injected directly -
	 * that is, it is only used to lazily create a CacheManager if one is not
	 * already provided.
	 *
	...
	/**
	 * Loads an existing EhCache from the cache manager, or starts a new cache if
	 * one is not found.
	 *
	 * @param name
	 ...
	 * Dependency Injection or programatically) prior to calling this method, this
	 * method does nothing.
	 * <p/>
	 ...
	 * {@code ehcache.xml} file at the root of the classpath. If one is not found,
	...
	private net.sf.ehcache.CacheManager ensureCacheManager() {
		try {
			if (this.manager == null) {
				...
				// using the CacheManager constructor, the resulting instance is _not_ a VM
				// singleton
				// (as would be the case by calling CacheManager.getInstance(). We do not use
				// the getInstance here
				// because we need to know if we need to destroy the CacheManager instance -
				// using the static call,
				// we don't know which component is responsible for shutting it down. By using a
				// single EhCacheManager,
				// it will always know to shut down the instance if it was responsible for
				// creating it.
	 ...
	 * component to also destroy the CacheManager instance, and it will not attempt
	 * to do so.
	 */
	public void destroy() {
